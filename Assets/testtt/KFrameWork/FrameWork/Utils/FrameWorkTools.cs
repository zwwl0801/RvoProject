...
public static class FrameWorkTools
{
    ...
    public static bool AddRange<T>(this HashSet<T> data, IEnumerable<T> items)
    {
        bool allAdded = true;
        var en = items.GetEnumerator();
        while(en.MoveNext())
        {
            if(!data.Add(en.Current))
            {
                allAdded =false;
            }
        }
    ...
    public static bool HasSame<T>(this List<T> list)
    {
    ...
    public static void Resize<T>(this List<T> list, int newCount, T val =default(T))
    {
        if (newCount <= 0)
        {
            list.Clear();
        ...
        else
        {
            ...
            while (list.Count < newCount) list.Add(val);
        }
    }
